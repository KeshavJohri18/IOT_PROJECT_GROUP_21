...
# Try to import tikzplotlib but don't fail module import if it is incompatible with the installed matplotlib
try:
    import tikzplotlib
    _HAS_TIKZ = True
except Exception:
...
def plot_draw_save(y, x, scores, trackers, plot_draw_styles, result_plot_path, plot_opts):
    # Do not use LaTeX rendering (requires external latex). Use Matplotlib's built-in mathtext instead.
    ...
    # No need to escape '%' when not using LaTeX
    ylabel = plot_opts['ylabel']
    xlim = plot_opts['xlim']
    ylim = plot_opts['ylim']

    # Use plain title string and set fontweight when displaying
    ...
    for id, id_sort in enumerate(index_sort):
        ...
        legend_text.append('{} [{:.1f}]'.format(disp_name, float(scores[id_sort].item() if hasattr(scores[id_sort], 'item') else scores[id_sort])))

    # Create legend without attempting LaTeX bolding
    try:
    ...
    # Save plot as PNG only (no tex/pdf required)
    try:
        ...
        print(f"Warning: failed to save plot PNG: {e}")
    plt.close(fig)
    
    # plt.draw() removed since we are saving files only
...
def plot_got_success(trackers, report_name):
    ...
    The tracker name in the experiment file should be set to the name of the report-0.00001-50-0.7-0.5 file for that tracker,
    e.g. DiMP50_report_2019_09_02_15_44_25 if the report-0.00001-50-0.7-0.5 is name DiMP50_report_2019_09_02_15_44_25.json
