...
class sglatrack(BaseTracker):
    def __init__(self, params, dataset_name):
        ...
        # Load full checkpoint (not weights-only) because the saved checkpoint contains custom objects
        # Do this only if you trust the checkpoint source (security risk: arbitrary code execution).
        ...
        # Choose device: prefer CUDA only if CUDA_VISIBLE_DEVICES is set and torch reports available GPUs.
        # Otherwise use CPU to avoid NVIDIA driver errors when no GPU is present.
        use_cuda = False
